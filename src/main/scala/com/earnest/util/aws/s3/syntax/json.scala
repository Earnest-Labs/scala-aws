...
final class JsonOps(val s3DS: S3DataSource) extends AnyVal {
  /**
    ...
    */
  def getJson[F[_], T: Decoder](key: String)(implicit F: Effect[F]): F[Option[T]] =
    F.handleError(
      s3DS.openStreamToFile(key) >>=
        (is => transformToString(is).map(s => parse(s).toOption.flatMap(_.as[T].toOption))))(_ => None)

  def upsertJson[F[_], T: Encoder](key: String, meta: T)(implicit F: Effect[F]): F[Unit] =
  ...
  private def transformToString[F[_]](is: InputStream)(implicit F: Effect[F]): F[String] = {
    F.bracket(F.delay(new BufferedReader(new InputStreamReader(is))))(br =>
...
trait ToJsonOps {
  implicit def toDataSourceOps(s3DS: S3DataSource): JsonOps =
    new JsonOps(s3DS)
}
